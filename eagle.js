...
fs.closeSync(fs.openSync(`./${argv.f}`, 'a+'))

const question = (i) => {
    return new Promise( res => rl.question(`What's your choice (Орел - 0 or Решка - 1)? - `, choice => {
        let coin = Math.floor(Math.random() * 2);
        fs.appendFile(path.normalize(`./${argv.f}`),`Move ${i}. Your choice: ${choice}.\n  Coin throw result: ${coin}\n\n`, err => { return })
        if(coin == choice)  { console.log('Вы выиграли!!!')
            fs.appendFile(path.normalize(`./${argv.f}`),`Number of attempts: ${i}\n----------------\n\n`, err => { return })
            res(true) }
